...
def _reload_jwt_modules():
    ...
    from api_promogg.auth import auth_facade, cookies, jwt_provider

    importlib.reload(auth_facade)
    importlib.reload(cookies)
    importlib.reload(jwt_provider)
    return auth_facade, cookies, jwt_provider
...
def test_jwt_provider_nao_ativo_por_padrao(monkeypatch):
    ...
    _auth_facade, _cookies, jwt_provider = _reload_jwt_modules()
...
def test_jwt_configuracao_padrao(monkeypatch):
    for env_name in constants.ENV_VARS:
        monkeypatch.delenv(env_name, raising=False)
    _auth_facade, _cookies, _jwt_provider = _reload_jwt_modules()
...
def test_cookies_specs_seguros(monkeypatch):
    ...
    _auth_facade, cookies, _jwt_provider = _reload_jwt_modules()
...
def test_producao_continua_sem_emissao_de_jwt(monkeypatch):
    ...
    auth_facade, _cookies, jwt_provider = _reload_jwt_modules()

    provider = jwt_provider.ExperimentalJWTProvider()
    facade = auth_facade.AuthCredentialFacade(provider)

    assert provider.is_enabled() is False
    with pytest.raises(auth_facade.AuthFacadeError):
...
def test_jwt_experimental_emite_somente_quando_chamado_explicitamente(monkeypatch):
    ...
    provider = jwt_provider.ExperimentalJWTProvider()
    facade = auth_facade.AuthCredentialFacade(provider)
    bundle = facade.issue_credentials(
        subject="usr_1",
        session_id="ses_1",
        signing_key="secret",
        roles=["Operador"],
